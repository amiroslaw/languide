...
public class DataLoader implements ApplicationRunner {
    ...
    @Autowired
    DataLoader(NotebookService notebookService, UserService userService, ArticleService articleService) {
...
//            notebooksSet.add(notebook);
        ...
        Translation translation = Translation.builder()
                .source("W końcu mam akt zgonu mojego ojca. Moja siostra była na tyle uprzejma, że wysłała jej zdjęcie, a ona prześle jej kopię pocztą. Myślałem, że urodził się w Hammond w stanie Indiana w jedynym szpitalu w okolicy.")
                ...
                .articleTranslation(
                        "I finally have a death certificate of my father. My sister was kind enough to email a photo of it and she will send a copy of it through the mail. I thought he was born in Hammond, Indiana, in the only hospital in the area.")
                .articleTranslation(
                        "However, the certificate says he was born in Illinois, just across the state border so it must have been a home birth. I made the change in the family tree to keep it accurate.")
                .build();
        translationRepository.save(translation);
        Dictionary dictionary = Dictionary.builder().build();

        Date date = Calendar.getInstance().getTime();
        Collection<Role> roleUser = Collections.singletonList(new Role("ROLE_USER"));
        ...
        Notebook publicNotebook = Notebook.builder().title("public").description("public notebook for no register user")
                .user(user2).build();
        notebookService.createOrUpdateNotebook(publicNotebook);

        Notebook notebook = Notebook.builder().title("DB notebook").description("first notebook for user").user(user)
                .build();
        ...
        Notebook notebookTest = Notebook.builder().title("test").description("first notebook for qwer").user(user2)
                .build();
        Notebook notebookTest2 = Notebook.builder().title("test private").description("second notebook for qwer")
                .user(user2).build();
